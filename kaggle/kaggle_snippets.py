"""
================================================================================
 Vehicle Damage Detection & Severity — KAGGLE snippets
================================================================================
Each block below is ONE Kaggle notebook cell. Run them top to bottom.

SETUP ON KAGGLE (do this once, before running cells):
  1. Create a new Kaggle Notebook. Settings > Accelerator > GPU T4 x2 (or P100).
  2. Add data (right panel "+ Add Input"):
       a) Search datasets for: cardd-with-yolo-annotations-images-labels
          -> attach it. It mounts at /kaggle/input/cardd-with-yolo-annotations-images-labels/
       b) Upload kaggle/upload_to_kaggle.zip as a NEW dataset (contains
          best_model.pt + your minor/moderate/severe crops). Note its mount path,
          e.g. /kaggle/input/<your-upload-dataset-name>/
  3. Set MYDATA in Snippet 1 to that upload mount path.

Kaggle notebooks already have ultralytics, tensorflow, torch, opencv — no pip needed.
The /kaggle/input paths are READ-ONLY; we write everything to /kaggle/working.
================================================================================
"""


# === SNIPPET 1 — Paths, GPU check, unpack the upload bundle ==================
import os, zipfile, shutil, torch
from pathlib import Path

CARDD = '/kaggle/input/cardd-with-yolo-annotations-images-labels'   # attached dataset
MYDATA = '/kaggle/input/CHANGE-ME'   # <-- EDIT: mount path of your uploaded zip dataset
WORK = '/kaggle/working'
os.chdir(WORK)
...
# If you uploaded the .zip itself (not pre-extracted), unzip it into working.
zips = [f for f in os.listdir(MYDATA) if f.endswith('.zip')] if os.path.isdir(MYDATA) else []
if zips:
    with zipfile.ZipFile(os.path.join(MYDATA, zips[0])) as z:
        z.extractall(WORK)
    BEST = os.path.join(WORK, 'best_model.pt')
    HAND = WORK
else:
    # Kaggle already extracted the dataset contents into MYDATA
    BEST = os.path.join(MYDATA, 'best_model.pt')
    HAND = MYDATA
...
print("CarDD train images:", len(os.listdir(os.path.join(CARDD, 'train/images'))))


# === SNIPPET 2 — Write the corrected 6-class data.yaml =======================
...
# === SNIPPET 3 — Validate the recovered detector ============================
# If this prints GOOD, SKIP snippet 4. If WEAK, run snippet 4.
...
try:
    ...
    print("\n>>> Weights look GOOD — skip snippet 4." if GOOD
          else "\n>>> Weights look WEAK — run snippet 4.")
...
# === SNIPPET 4 — Retrain detector (ONLY if snippet 3 said WEAK; ~30 min) =====
model = YOLO('yolov8n.pt')   # Kaggle auto-downloads this
model.train(
...
detector = YOLO(BEST)
print("Saved new best_model.pt to /kaggle/working")


# === SNIPPET 5 — Severity labeling rule ======================================
...
# === SNIPPET 6 — Auto-label extra severity crops (weak supervision) ==========
...
# === SNIPPET 7 — Build train (auto + hand) and val (hand only) splits ========
import random
...
for c in CLASSES:
    src = Path(HAND) / c
    files = [f for f in src.iterdir() if f.suffix.lower() in ('.jpg', '.jpeg', '.png')] if src.is_dir() else []
...
# === SNIPPET 8 — Train the severity classifier (MobileNetV2) =================
...
# === SNIPPET 9 — Confusion matrix on the trusted hand-labeled val set ========
y_true, y_pred = [], []
for x, y in val_ds:
...
for i, t in enumerate(tests):
    ...
    display(IPyImage(o))
# Download demo_0.jpg, severity_model.h5, best_model.pt from the /kaggle/working
# panel on the right, then paste your metrics back to finish the README.
